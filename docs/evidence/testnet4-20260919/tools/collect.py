"""Read-only collector: Node views, scheduler/journal rows, Bitcoin verification."""
import json, subprocess, sys, os, base64, urllib.request, datetime
OUT=sys.argv[1]; U,P=sys.argv[2],sys.argv[3]; ONLY=set(sys.argv[4:])  # swap ids of this run; the Nodes hold older ones too
...
def core(m,*a):
    q=urllib.request.Request("http://127.0.0.1:48332/",data=json.dumps({"jsonrpc":"1.0","id":1,"method":m,"params":list(a)}).encode(),
    ...
    for t in sorted(txids):
        try:
            ...
            rec["bitcoin_verification"].append({"txid":t,"confirmations":x.get("confirmations",0),"block_height":h.get("height"),"block_hash":x.get("blockhash"),
                ...
                "witness_items":[len(i.get("txinwitness",[])) for i in x["vin"]]})
